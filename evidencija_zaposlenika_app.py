    ...
    try:
        ...
        c.execute('''INSERT INTO employees
                     (name, oib, address, birth_date, hire_date,
                      ...
                      children_under15, sole_caregiver, previous_experience_days)
    ...
    try:
        c.execute('''UPDATE employees 
                  ...
                  (data['name'], data['oib'], data['address'], data['birth_date'],
                   ...
                   int(data['sole']), int(data['previous_experience_days']), emp_id))
        conn.commit()
        return True
    except Exception as e:
    ...
    try:
        ...
        with st.form("employee_form"):
            ...
            with col1:
                ...
                previous_experience_days = st.number_input("Staž prije (dani)", 
                    min_value=0, value=selected_employee.get('previous_experience_days', 0) if selected_employee else 0)
                 (data['name'], data['oib'], data['address'], birth_date,
                  hire_date, next_phys, next_psy,
                  data['invalidity'], data['children'], data['sole'],
                  data['previous_experience_days']))
            ...
            if st.form_submit_button("Spremi"):
                try:
                    data = {
                        ...
                        'previous_experience_days': previous_experience_days
                    }
